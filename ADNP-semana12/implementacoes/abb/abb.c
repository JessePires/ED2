...
void ABB_Inserir_recursiva (ABB** A, int chave) {
  if ((*A) == NULL) {
...
void ABB_Inserir_iterativa (ABB **A, int chave) {
  ABB *novo, *aux, *pai;
  
  novo = ABB_Criar(NULL, NULL, chave);
  ...
  if (*A == NULL) {
    *A = novo;
    ...
    while (aux != NULL) {
      pai = aux;
      aux = pai->chave > chave ? pai->esq : pai->dir;
    }

    aux = novo;
    if (pai->chave > chave) pai->esq = aux;
    else pai->dir = aux;
  }
}

void ABB_Inserir (ABB **A, int chave) {
  // ABB_Inserir_recursiva(A, chave);
  ABB_Inserir_iterativa(A, chave);
}
